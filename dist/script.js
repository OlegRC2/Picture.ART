...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_sliders__WEBPACK_IMPORTED_MODULE_1__["default"])('.feedback-slider-item', 'horizontal', '.main-prev-btn', '.main-next-btn'); // функция работы горизонтального слайдера

  Object(_modules_sliders__WEBPACK_IMPORTED_MODULE_1__["default"])('.main-slider-item', 'vertical'); // функция работы вертикального слайдера

  Object(_modules_forms__WEBPACK_IMPORTED_MODULE_2__["default"])(); // функция для работы с формами

  Object(_modules_mask__WEBPACK_IMPORTED_MODULE_3__["default"])('[name="phone"]'); // функция для работы маски инпута

  Object(_modules_checkTextInputs__WEBPACK_IMPORTED_MODULE_4__["default"])('[name="name"]'); // функция для валидации инпута имени

  Object(_modules_checkTextInputs__WEBPACK_IMPORTED_MODULE_4__["default"])('[name="message"]'); // функция для валидации инпута комментария

  Object(_modules_showMoreStyles__WEBPACK_IMPORTED_MODULE_5__["default"])('.button-styles', '#styles .row'); // функция подгрузки карточек (в блоке с id=syle есть блок с классом row)
...
var calc = function calc(size, material, options, promocode, result) {
  // функция для калькулятора. size - select размера картины, material - select материала, options - select доп. услуг, promocode - инпут для промокод, result - окно для вывода итоговой цены
  ...
  var sum = 0; // переменная для рассчетной суммы
  ...
  var calcFunction = function calcFunction() {
    ...
    if (sizeBlock.value == '' || materialBlock.value == '') {
      // если первый селект равен дефолтному значению (пустая строка) или второй, то
...
var checkTextInputs = function checkTextInputs(selector) {
  // функция для валидации инпутов имени и комментария
  var txtInputs = document.querySelectorAll(selector); // берем все нужные инпуты

  txtInputs.forEach(function (input) {
    // перебираем все инпуты
    input.addEventListener('keypress', function (e) {
      // навешиваем обработчик нажатия кнопки, передаем объект события в функцию
      if (e.key.match(/[^а-яё 0-9]/ig)) {
        // e.key - кнопка, которую нажали. match - ищем соответсвие на регулярное выражение, весь русский алфавит (буква ё стоит в нем после я) и цифры. Флаг i - в любом регистре. ^ - значение этого символа в [] скобках - отрицание
        e.preventDefault(); // отменяем стандартное поведение браузера
      }
    });
    input.addEventListener('input', function () {
      // навешиваем обработчик собития инпут
      if (input.value.match(/[a-z]/ig)) {
        // если в инпуте каким-то образом появились латинские буквы, например автозаполнение, то 
...
var filter = function filter() {
  // функция для работы блока с фильтрами
  ...
  var typeFilter = function typeFilter(markType) {
    // функция скрытия и показа всех элементов
    ...
    if (markType) {
      // если в функцию передано то, что хотим отобразить, то
      ...
      // если в функцию ничего не передано, то
  ...
  btnAll.addEventListener('click', function () {
    // навешиваем на кнопку всего контента обработчик клика
  ...
  menu.addEventListener('click', function (e) {
    // навешиваем на блок с кнопками обработчик клика
    var target = e.target; // сокращаем запись цели события

    if (target && target.tagName == 'LI') {
      // если событие существует и блок в который кликнули li (обязательно в верхнем регистре)
      items.forEach(function (btn) {
        return btn.classList.remove('active');
      }); // перебираем все кнопки и убираем у каждой класс активности
...
var forms = function forms() {
  ...
  var path = {
    ...
    // путь для отправки сообщения с картинкой пользователя
  ...
  var clearInputs = function clearInputs() {
    ...
    upload.forEach(function (item) {
      // берем каждый инпут для загрузки файлов
      item.previousElementSibling.textContent = 'Файл не выбран'; // задаем ему нужное значение
    });
  };

  upload.forEach(function (item) {
    // перебираем все инпуты для загрузки файлов
    item.addEventListener('input', function () {
      // навешиваем на каждый инпут обработчик, который страбатывает при "вводе" чего-либо в инпут, в данном случае при загрузке файла
      console.log(item.files[0]); // выводим первый файл, загруженный в инпут

      var dots; // переменная либо будет содержать ... либо нет

      var arr = item.files[0].name.split('.'); // берем имя первого файла и split делим его на 2 части, разделитель деления . Образуется массив с 2 значениями
  ...
  form.forEach(function (item) {
    ...
    item.addEventListener('submit', function (e) {
      ...
      var statusMessage = document.createElement('div'); // создаем элемент, который потом будет выводится пользователю с сообщением с картинкой
      ...
      var statusImg = document.createElement('img'); // создаем элемент img, в нем будет изображение отображающее статус отправки
      ...
      var textMessage = document.createElement('div'); // создаем элемент, который потом будет выводится пользователю с текстовым сообщением
      ...
      var api; // переменная для динамического пути отправки
      ...
      Object(_services_requests__WEBPACK_IMPORTED_MODULE_6__["postData"])(api, formData) // отправялем запрос по адресу api      
      .then(function (res) {
...
var mask = function mask(selector) {
  ...
  var setCursorPosition = function setCursorPosition(pos, elem) {
    // функция для установки курсора в нужное место. pos - позиция, elem - элемент, на котором будут срабатывать
    elem.focus(); // установили фокус на элементе

    if (elem.setSelectionRange) {
      // если у элемента есть метод setSelectionRange. В старых браузерах его нет, это так сказать самописный полифилл
      ...
      // если элемент поддерживает метод createTextRange. По сути то же самое, что и setSelectionRange
      var range = elem.createTextRange(); // создали диапазон
  ...
  function createMask(event) {
    // функция создания маски, внутрь передается событие из обработчика
    var matrix = '+7 (___) ___ __ __',
        // маска номера телефона
    i = 0,
        // итератор 
    def = matrix.replace(/\D/g, ''),
        // заменяем в маске все "не цифры" (/\D/g) пустой строкой
    val = this.value.replace(/\D/g, ''); // заменяем в самом инпуте все "не цифры" (/\D/g) пустой строкой

    if (def.length >= val.length) {
      // если длина строки def больше или равна строке val, то (для того, если пользователь удаляет маску, она возвращалась)
      val = def; // передаем в val исходное значение
    }

    this.value = matrix.replace(/./g, function (a) {
      // передаем в инпут, в текущий символ, значение маски. В ней берем каждый символ (/./g) и выполняем функцию. В а будет подставляться каждый символ из /./g
    ...
    if (event.type === 'blur') {
      // если событие равоно blur - т.е. пользователь вышел из фокуса инпута (нажал вне инпута)
      if (this.value.length == 2) {
        // если в инпуте 2 символа, то
        this.value = ''; // очищаем инпут
      }
    } else {
      // если тип события другой (фокус, инпут)
      setCursorPosition(this.value.length, this); // функция для установки курсора в нужное место
    }
  }

  var inputs = document.querySelectorAll(selector); // берем нужные инпуты
...
/* harmony default export */ __webpack_exports__["default"] = (mask); // экспортируем функцию
...
var scrolling = function scrolling(upSelector) {
  // функция для плавного скроллинга на сайте. upSelector - стрелка вверх в углу страницы
  var upElem = document.querySelector(upSelector); // получаем стрелку вверх

  window.addEventListener('scroll', function () {
    // вешаем событие скролла на окно
    if (document.documentElement.scrollTop > 1650) {
      // если расстояние, которое проскроллено сверху больше 1650 пикселей, то
      upElem.classList.add('animated', 'fadeIn'); // отображаем стрелку с анимацией

      upElem.classList.remove('fadeOut'); // удаляем класс скрытия
    } else {
      // если пользователь проскрорлил обратно и расстояние стало менее 1650 пикселей, то
  ...
  var links = document.querySelectorAll('[href^="#"]'),
      // берем все ссылки начинающиеся с #
  ...
  links.forEach(function (link) {
    ...
    link.addEventListener('click', function (event) {
      ...
      var widthTop = document.documentElement.scrollTop,
          // значение сколько уже пролистано
      hash = this.hash,
          // получаем хэш текущего элемента
      toBlock = document.querySelector(hash).getBoundingClientRect().top,
          // получаем верхнюю границу элемента к которому будет прокрутка
      ...
      function step(time) {
        ...
        if (r != widthTop + toBlock) {
          // если еще скролл не дошел до нужного элемента, то
          requestAnimationFrame(step); // продолжаем выполнение анимации
        } else {
          // если значения стали равны, значит анимация должна закончится
          location.hash = hash;
        }
      }
    });
...
/* harmony import */ var _services_requests__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../services/requests */ "./src/js/services/requests.js");
 ...
 // импортируем функцию отправки запроса на получение данных с сервера

var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  // функция показа скрытых карточек, trigger - кнопка, styles - карточки (для первого варианта вместо wrapper), wrapper - обертка куда будут помещаться карточки
  var btn = document.querySelector(trigger); // получаем кнопку
  ...
  btn.addEventListener('click', function () {
    // навешиваем обработчик клика на кнопку, внутри используем именно function, чтобы срабатывал контекст вызова кнопки 
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["getResourse"])('http://localhost:3000/styles') // получаем данные с нужного адреса (нужно включить json-server) можно сразу прописать конечный адрес базы данных, который будет использоваться в готовом проекте. Gulp переместит файл базы данных в assets/db.json - это и можно прописать в адресе, но далее в следующей строке потребуется изменить createCards(res.styles). Урок 17, 23 минута
    .then(function (res) {
      return createCards(res);
    }) // при положительном ответе сервера выполняем функцию создания карточек
    .catch(function (error) {
      return console.log(error);
    }); // при ошибке выводим ошибку в консоль

    this.remove(); // удаляем кнопку, в обработчике прописана не стрелочная функция, т.к. контекст в стрелочной не сработает
  });

  function createCards(response) {
    // функция создания карточек. response - данные, полученные с сервера
    response.forEach(function (item) {
      // перебираем ответ с сервера, т.к. это массив
      var card = document.createElement('div'); // создаем элемент для карточки
...
var sliders = function sliders(slides, dir, prev, next) {
  // slides - сами слайды, dir - направление движениея слайдов (горизонтально или вертикально), prev и next стрелки 
  var slideIndex = 1,
      // переменная отображающая текущий слайд
  paused = false; // переменная для остановки автопереключения при наведении мыши на слайдер

  var items = document.querySelectorAll(slides); // получаем все слайды
  ...
  try {
    // для исключения поломки кода используем конструкцию try/catch. Если кнопки не были переданы весь код не ляжет
    ...
    nextBtn.addEventListener('click', function () {
      // навешиваем обработчик клика на кнопку dgthtl
  ...
  function activateAnimation() {
    ...
    if (dir === 'vertical') {
      ...
      paused = setInterval(function () {
        // переключение сладера автоматически, присваиваем переменной для последующей остановки
      ...
      // иначе слайдер горизонтальный (можно передать любое значение)
      paused = setInterval(function () {
        // переключение сладера автоматически, присваиваем переменной для последующей остановки
        plusSlides(1); // переключаем слайд
  ...
  items[0].parentNode.addEventListener('mouseenter', function () {
    // навешиваем обработчик наведения мыши на родителя слайда, т.е. на весь слайдер
    clearInterval(paused); // останавливаем автопереключение
  });
  items[0].parentNode.addEventListener('mouseleave', function () {
    // навешиваем обработчик отвода мыши с родителя слайда, т.е. на весь слайдер
